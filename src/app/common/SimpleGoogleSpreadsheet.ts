...
export class SimpleGoogleSpreadsheet {
  ...
  private _isBlankRow (rowData: CellType): boolean {
    let result = true
    rowData.forEach((col) => {
      if (col !== '') result = false
    })
  ...
  private _blankRowDel (data: Array<CellType> | null | undefined): Array<CellType> | null {
    if (!data) return null
    return data.filter((row) => !this._isBlankRow(row))
  }

  private _blankColDel (rowData: CellType): CellType | null {
  ...
  doGetLastRow (firstRow: number, col: number) {
    ...
    if (data && data.length < 2) return firstRow + 1

    const sheet = this.sheet
    const row = sheet && sheet.getRange(firstRow, col).getNextDataCell(SpreadsheetApp.Direction.DOWN).getRow() + 1
    return row
  }
  ...
  doGetLastCol (row: number, firstCol: number) {
    ...
    const blank = data && this._blankColDel(data[0])
    if (blank && blank.length < 2) return firstCol + 1

    const sheet = this.sheet
    const col = sheet && sheet.getRange(row, firstCol).getNextDataCell(SpreadsheetApp.Direction.NEXT).getColumn() + 1
    return col
  }
